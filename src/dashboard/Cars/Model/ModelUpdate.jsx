...
import { updateModel } from "../../../store/cars/modelSlice";

const ModelUpdate = ({ isModelEditFrom, setIsModelEditFrom, editData }) => {
  const dispatch = useDispatch();

  const [formData, setFormData] = useState({
    ...editData,
    fuelType: new Set(editData.fuelType),
    transmission: new Set(editData.transmission),
  });
  ...
  const submitHandler = (e) => {
    ...
    dispatch(updateModel(formattedData,setIsModelEditFrom)); 
  };

  return (
    <div className="rounded-sm border border-stroke bg-white px-5 pt-6 pb-2.5 shadow sm:px-7.5 xl:pb-1">
      <div className="flex justify-between mb-6">
        <div className="text-xl font-bold uppercase">Update Model</div>
        <IoIosCloseCircleOutline
          className="text-2xl cursor-pointer"
          onClick={() => setIsModelEditFrom(!isModelEditFrom)}
        />
      </div>
      ...
      <form className="max-w-md mx-auto" onSubmit={submitHandler}>
        <div className="relative z-0 w-full mb-5 group">
          <select
            ...
            defaultValue={formData.carBrand}
            disabled
              ...
              (fuel) => (
                <label key={fuel} className="inline-flex items-center px-4 cursor-pointer">
                  <input
                    ...
                    name="fuelType"
                    value={fuel}
                    checked={formData.fuelType.has(fuel)}
            ...
            {["Manual", "Automatic"].map((transmission) => (
              ...
              >
                <input
                  ...
                  name="transmission"
                  value={transmission}
                  checked={formData.transmission.has(transmission)}
                  onChange={handleCheckboxChange}
                  className="mr-2"
                />
                {transmission}
